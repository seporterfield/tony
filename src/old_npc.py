...
class NPC:
    ...
    def make_description(
        cls, name, username, age, gender, setting, occupation, personality, context
    ):
        return (
            f" Name: {name}, Username: {username}, Age: {age}, Gender: {gender}, "
    ...
    def __init__(
        ...
        self.description = NPC.make_description(
            name=name,
            username=username,
            age=age,
            gender=gender,
    ...
    def make_system_message(self) -> str:
        return (
            f"You are {self.username}, an NPC in a videogame that takes place in a Discord server. "
            + self.description
    ...
    async def is_chat_relevant(self) -> bool:
        ...
        if self.messages:
            ...
            raise discord.ClientException(
                "Message history was not able to update properly."
            )
        ...
        if new_msg.reference != None:
            replied_msg: discord.Message = await new_msg.channel.fetch_message(
    ...
    def make_completion_messages(
        self, system_prompt: str, mode: NPCPromptMode = NPCPromptMode.RELEVANCE_CHECKER
    ) -> list[dict]:
        ...
        if mode == NPCPromptMode.RELEVANCE_CHECKER:
            ...
            for message in self.messages:
                content = message.content.replace(
                    str(self.client.user.id), self.username
                )
                chat_history += f"{message.author.name}: {content}" + "\n"
            completion_messages.append({"role": "user", "content": chat_history})
        elif mode == NPCPromptMode.NPC:
            for message in self.messages:
                role = (
                    "assistant" if self.client.user.id == message.author.id else "user"
                )
            ...
            raise discord.ClientException(
                "chat completion mode set incorrectly. use either 'npc' or 'relevance_checker'."
            )
    ...
    async def prompt_openai(self, completion_messages: list[dict]) -> str:
        ...
        for i in range(MAX_OPENAI_API_CALL_ATTEMPTS + 1):
            if i == MAX_OPENAI_API_CALL_ATTEMPTS:
                self.logger.warning(
                    "MAX CALLS TO OPENAI API REACHED. SHUTTING OFF BOT CLIENT!"
                )
                await self.client.close()
                break

            api_process: Process = Process(
                target=self.api_call_openai, args=(completion_messages, response_queue)
            )
            api_process.start()
    ...
    async def prompt(self) -> str:
        completion_messages = self.make_completion_messages(
            self.system_message, mode=NPCPromptMode.NPC
        )
        return await self.prompt_openai(completion_messages)
